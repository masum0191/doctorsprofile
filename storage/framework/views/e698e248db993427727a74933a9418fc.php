...
<?php
    ...
    if (\App\Support\DatabaseAvailability::check()) {
        try {
            $databaseReady = true;
            ...
            $featuredDoctors = User::where('role', 'tenant')
                ...
                ->get();
            $doctor = User::first();
            $posts = DoctorPost::where('is_published', 1)->latest()->take(9)->get();
        } catch (\Throwable) {
            $databaseReady = false;
        }
    }
            ...
            <div class="bg-white rounded-2xl sm:rounded-3xl shadow-[0_20px_60px_-15px_rgba(0,0,0,0.1)] p-4 sm:p-6 md:p-8 max-w-5xl mx-auto border border-[#318069]/20">
                ...
                <div class="flex flex-col md:grid md:grid-cols-12 gap-3 md:gap-4">
                    ...
                    <div class="w-full md:col-span-3 relative">
                        <div id="locationSelector" data-location-fixed="false" role="button" tabindex="0" class="flex items-center bg-gray-100 rounded-xl sm:rounded-2xl px-3 sm:px-5 py-3 sm:py-4 border-2 border-transparent focus-within:border-[#318069] focus-within:bg-white transition-all cursor-pointer">
