        ...
        <div class="max-w-6xl mx-auto">
            ...
            <div id="step1" class="step-content active">
                ...
                <form id="doctorRegistrationForm" method="post" action="<?php echo e(route('doctor.store')); ?>"
                    ...
                    <input type="hidden" name="country" id="country" value="<?php echo e($initialCountry); ?>">
                    <input type="hidden" name="currency_code" id="currency_code" value="<?php echo e($initialCurrencyCode); ?>">
                            ...
                            <div class="mb-4 sm:mb-6 md:mb-8">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
                                    <button type="button" id="sslcommerz-payment-card" onclick="selectPaymentMethod('sslcommerz', this)"
                                        class="payment-method-card flex flex-col items-center text-center p-4 sm:p-6 border-2 border-gray-200 rounded-xl hover:border-[#318069] hover:bg-[#318069]/5 transition-all <?php echo e($initialSslcommerzAvailable ? '' : 'hidden'); ?>"
                                        <?php if(!$initialSslcommerzAvailable): ?> disabled <?php endif; ?>>
                                    ...
                                    <button type="button" id="stripe-payment-card" onclick="selectPaymentMethod('stripe', this)"
                                        ...
                                        <h6 class="font-bold text-gray-900 text-xs sm:text-sm">International Payment</h6>
                                        <p class="text-gray-600 text-xs mt-1 hidden sm:block">Cards and international payments</p>
                                    </button>
                                </div>
                                ...
                                <div id="stripeInstructions" class="mt-4 sm:mt-6 hidden">
                                    <div class="border-2 border-indigo-200 bg-indigo-50 rounded-xl p-3 sm:p-6">
                                        <div class="flex items-start gap-2 sm:gap-3">
                                            <i class="ri-information-line text-indigo-600 text-lg sm:text-xl flex-shrink-0"></i>
                                            <div class="flex-1">
                                                <h6 class="font-bold text-sm sm:text-base text-gray-900 mb-1 sm:mb-2">Stripe Payment Process</h6>
                                                <p class="text-xs sm:text-sm text-gray-700">You will be redirected to Stripe secure checkout for international payments.</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
        ...
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(
                ...
                function error(error) {
                    ...
                    if (countryField && !countryField.value) countryField.value = '';
                }, {
...
function useDomainSuggestion(suggestion) {
    ...
    function selectPackage(packageId) {
        ...
        if (isFreePackage) {
            ...
            if (paymentMethodField) {
                selectedPaymentMethod = preferredOnlinePaymentMethod();
    ...
    function validateStep4() {
        ...
        if (!['sslcommerz', 'stripe'].includes(selectedPaymentMethod)) {
            alert('Please select an available online payment method.');
            return false;
        }
    ...
    function selectPaymentMethod(method, trigger = null) {
        ...
        const activeTrigger = trigger || (typeof event !== 'undefined' ? event.currentTarget : null);
        if (activeTrigger) {
            activeTrigger.classList.add('selected');
        } else {
            const activeCard = method === 'sslcommerz'
                ? document.getElementById('sslcommerz-payment-card')
                : document.getElementById('stripe-payment-card');
        ...
        if (method === 'sslcommerz') {
            ...
            const stripeInstructions = document.getElementById('stripeInstructions');
            if (stripeInstructions) stripeInstructions.classList.remove('hidden');
        }
    ...
    function hideAllPaymentInstructions() {
        ...
        if (stripeInstructions) stripeInstructions.classList.add('hidden');
    }
    ...
    function updatePaymentUI() {
        ...
        if (isFreePackage || totalAmount <= 0) {
            ...
            if (sslCard) {
                sslCard.style.display = allowSslcommerz ? '' : 'none';
                sslCard.disabled = !allowSslcommerz;
                sslCard.classList.toggle('hidden', !allowSslcommerz);
            }

            if (stripeCard) {
                stripeCard.style.display = allowSslcommerz ? 'none' : '';
                stripeCard.disabled = allowSslcommerz;
                stripeCard.classList.toggle('hidden', allowSslcommerz);
            }

            selectPaymentMethod(preferredOnlinePaymentMethod());
